...
export default {
  ...
  methods: {
    ...
    init: function() {
      let array = [];
      for (let i = 0; i < this.iframeData.user.role.length; i++) {
        array.push(this.iframeData.user.role[i]._id);
      }
      this.values = array;
    },
    /**
    ...
    try {
      ...
      if (result.length > 0) {
        this.headers = [
          ...
          { text: "Describe", value: "role_describe" },
